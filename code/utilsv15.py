...
def optimize_sap_logic(syntax_string, known_canon):
    ...
    reverse_map = {}
    ghost_notes = []
    # FIX: Wir nutzen eine Liste für den Counter, um Scope-Probleme zu vermeiden
    counter = [0] 

    # --- 3. Maskierung für SymPy vorbereiten ---
    # Dollarzeichen entfernen (z.B. $self -> self)
    tagged_syntax = tagged_syntax.replace('$', '')
    
    # Erzwinge Leerzeichen um Operatoren, damit "C1and" zu "C1 and" wird
    tagged_syntax = re.sub(r'(?i)\b(and|or|not)\b', r' \1 ', tagged_syntax)
    
    # Alle Bedingungen maskieren (inkl. unmaskierter Zahlen wie = 0)
    # Nutzt COND_X anstatt CX, um Überschneidungen mit echten Merkmalen zu vermeiden
    condition_pattern = r'((?:not\s+)?specified\s+(?:\[UNCLASSIFIED_\w+\]|\b[A-Za-z_][A-Za-z0-9_]*\b)|(?:\[UNCLASSIFIED_\w+\]|\b[A-Za-z_][A-Za-z0-9_]*\b)\s*(?:=|<>|!=|ne|eq|<|>|>=|<=|in)\s*(?:\'.*?\'|\(.*?\)|-?\d+(?:\.\d+)?))'
    
    def mask_condition(match):
        cond = match.group(0)
        var_name = f"COND_{counter[0]}"
    ...
    for var_name, original_cond in mask_map.items():
        if "[UNCLASSIFIED" in original_cond:
            ...
            # Ersetze die Variable (z.B. COND_0) direkt durch True/False
    ...
    masked_syntax = re.sub(r'\band\b', '&', masked_syntax, flags=re.IGNORECASE)
    masked_syntax = re.sub(r'\bor\b', '|', masked_syntax, flags=re.IGNORECASE)
    masked_syntax = re.sub(r'\bnot\b', '~', masked_syntax, flags=re.IGNORECASE)

    # --- 5. SymPy Logik-Kürzung & Re-Assembly ---
    final_syntax = ""
    try:
        expr = sympy.sympify(masked_syntax, locals={k.name: k for k in reverse_map.keys()})
        ...
        # SMART FALLBACK: SymPy crasht (z.B. wegen fehlender Klammern in SAP). 
        ...
        # Repariere die Operatoren für die Ausgabe
        final_syntax = final_syntax.replace('&', 'AND').replace('|', 'OR').replace('~', 'NOT ')
...
# ==========================================
# NEU: DETERMINISTISCHER TABLE BUILDER (Ersetzt Agent 1)
# ==========================================
def create_heuristic_table_plan(char_dict):
    """
    Python-Heuristik zur Entscheidung TABLE vs NO_TABLE.
    Wendet striktes Schema-Matching an (Option A).
    """
    ...
    for val in values:
        ...
        original_mapping.append({"wert": code, "logik": logic})

        # 1. Kill-Operatoren für Tabellen
        ...
        if is_table:
            # Kommentare und Klammern entfernen
            clean_logic = re.sub(r'/\*.*?\*/', '', logic).strip()
            clean_logic = clean_logic.replace('(', '').replace(')', '')
            ...
            row_dict = {}
            for part in parts:
                if not part or part.upper() in ["ALWAYS_TRUE", "TRUE", "FALSE"]:
                    continue
                
                # Prüfe auf Var = 'Val'
                eq_match = re.match(r'^([A-Za-z0-9_]+)\s*=\s*(.+)$', part)
                if eq_match:
                    var = eq_match.group(1).upper()
                    val_str = eq_match.group(2).replace("'", "").strip()
                    row_dict[var] = val_str
                    continue
                    
                # Prüfe auf Var IN ('Val1', 'Val2')
                in_match = re.match(r'^([A-Za-z0-9_]+)\s+IN\s*\((.+)\)$', part, flags=re.IGNORECASE)
                if in_match:
                    var = in_match.group(1).upper()
                    val_str = in_match.group(2).replace("'", "").replace(" ", "")
                    row_dict[var] = val_str
                    continue
                
                # Ignoriere "SPECIFIED" (Meist GHOST-Logic, brauchen wir nicht in Tabellenspalten)
                if "SPECIFIED" in part.upper():
                    continue

                # Wenn wir hier landen, war die Syntax zu komplex -> Keine Tabelle!
                is_table = False
                break
                
            if is_table:
                all_value_dicts.append({"code": code, "dict": row_dict})

    # 2. OPTION A: STRIKTES SCHEMA MATCHING (Keine Logische Subsumtion)
    first_cols = set()
    if is_table and all_value_dicts:
        first_cols = set(all_value_dicts[0]["dict"].keys())
        for item in all_value_dicts:
            if set(item["dict"].keys()) != first_cols:
                is_table = False # Spalten weichen ab! -> Code statt kaputter Tabelle
                break
    else:
        is_table = False

    if is_table and len(first_cols) == 0:
        is_table = False

    # 3. JSON Output für Clustering und Synthesizer bauen
    plan = {
        "zielmerkmal": char_name,
        "decision": "TABLE" if is_table else "NO_TABLE",
        "bedingungs_spalten": [],
        "zeilen_daten": [],
        "if_constraints_text": "Deterministisch durch Python Heuristik als valides Schema erkannt." if is_table else "Asymmetrische Spalten oder komplexe Operatoren -> Standalone Route.",
        "original_code_mapping": original_mapping
    }

    if is_table:
        cols = sorted(list(first_cols))
        plan["bedingungs_spalten"] = cols
        for item in all_value_dicts:
            row = []
            for c in cols:
                row.append(item["dict"][c])
            row.append(item["code"]) # Zielwert ganz nach hinten
