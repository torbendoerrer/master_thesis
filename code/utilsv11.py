...
def optimize_sap_logic(syntax_string, known_canon):
    """
    Maskiert SAP Syntax, wendet De Morgan an, löst Unclassified-Logik auf 
    und dokumentiert die Auflösung transparent im String.
    """
    if not syntax_string or syntax_string.strip() == "":
        return ""

    # 1. Tobi's Regeln für Geister-Merkmale taggen
    ...
    # 2. Alle Bedingungen maskieren (inkl. unmaskierter Zahlen wie = 0)
    ...
    # 3. Geister-Merkmale vor SymPy zu True/False auswerten und LOGGEN!
    for var_name, original_cond in mask_map.items():
        if "[UNCLASSIFIED" in original_cond:
            ...
            # Variable im Mask-String durch True/False ersetzen
            masked_syntax = re.sub(fr'\b{var_name}\b', bool_val, masked_syntax)
            # Aus der Entmaskierungs-Map löschen, da es aufgelöst wurde
    ...
    # 4. SymPy Logik-Kürzung & Re-Assembly
    try:
        ...
        # 5. DIE GHOST-BREADCRUMBS ANHÄNGEN
        if ghost_notes:
        ...
        # Im Test-Modus werfen wir hier lieber einen echten Fehler, damit du siehst was crasht
        print(f"⚠️ AST Parser Warnung für '{syntax_string}': {e}.")
        return tagged_syntax
...
def heavy_preprocessing(char_dict, known_canon):
    ...
    3. Jagt die Syntax durch den AST Optimizer (inkl. Ghost-Log).
    """
    import copy
