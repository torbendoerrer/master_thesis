...
# ==========================================
# 2. WISSENSBASIS LADEN (Knowledge Injection / RAG)
# ==========================================
...
try:
    ...
    print("⚠️ Warnung: 'avc_guidelinesv2.md' nicht gefunden. Agent hat kein spezifisches Domänenwissen!")
...
def avc_format_validator(avc_code: str) -> str:
    ...
    return "VALIDATION FAILED:\n" + "\n".join(errors) + "\nBitte korrigiere den Code zwingend nach den Vorgaben im Handbuch!"
...
architect = Agent(
    ...
    Deine Aufgaben:
    1. Herleitung: Erkläre kurz in Textform, welche Werte du aggregierst (weil sie identische Bedingungen haben) und welche Ausreißer du außerhalb der Tabelle behandelst.
...
synthesizer = Agent(
    ...
    Darunter schreibst du "AVC CODE:" und fügst den reinen, sauberen AVC-Constraint-Code ein (TABLE-Aufruf und ggf. Einzeiler für Ausreißer). Bilde die Inhalte der Tabelle NICHT im Code nach!''',
    verbose=True,
...
def process_lovc_json(input_file, output_file):
    ...
    for cls in classes:
        ...
        for char in characteristics:
            ...
            task_code = Task(
                description=f'Nimm Herleitung und Tabelle des Architekten für {char_name}. Erstelle den AVC-Code nach Vorgabe deines Handbuchs (KEINE IF-Bedingungen für Tabellenwerte!). Prüfe den Code mit dem Validator. Präsentiere am Ende Herleitung, Tabelle und Code zusammen.',
...
if __name__ == "__main__":
    # Passe die Dateinamen an deine lokalen Dateien an
    INPUT_JSON = "Beispiel1_Vorbedingungen.json"
    OUTPUT_FILE = "MAS_Visuelle_Vorschlaege_V2.txt"
