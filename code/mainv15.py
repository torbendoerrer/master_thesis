...
from utilsv15 import extract_json_from_output, create_excel_dashboard, extract_known_canon, heavy_preprocessing, create_heuristic_table_plan
from agentsv15 import global_joiner, synthesizer, avc_validator

def process_lovc_json(full_model_file, test_slice_file, output_file, debug_file="debug_preprocessed.json"):
    
    # =================================================================
    # VORBEREITUNG: Lexikon (known_canon) aus dem Full-Model erstellen
    # =================================================================
    ...
    # =================================================================
    # PHASE 1: MAP (Pre-Processing & Python Heuristik)
    # =================================================================
    print("\n" + "="*50)
    ...
    for cls in classes:
        for i, char in enumerate(cls.get("characteristics", [])):
            ...
            # SCHRITT 1A: Die "Waschmaschine" (SymPy & Ghost-Logic)
            processed_char = heavy_preprocessing(char, known_canon)
            # Updaten des JSONs für evtl. späteres Debugging
            cls["characteristics"][i] = processed_char 
            
            # SCHRITT 1B: Die fehlerfreie, deterministische Python-Heuristik (Tabelle vs. Code)
            parsed_plan = create_heuristic_table_plan(processed_char)
            local_plans.append(parsed_plan)

    # Debug-Dump Phase 1
    try:
        with open("debug_1_local_plansv15.json", "w", encoding="utf-8") as f:
    ...
    for plan in local_plans:
        if plan.get("decision") == "NO_TABLE":
            ...
            # Cluster-Key erzeugen (Alphabetisch sortierte Spaltennamen)
    ...
    # Debug-Dump Phase 2
    try:
        with open("debug_2_clustersv15.json", "w", encoding="utf-8") as f:
    ...
    print(f"🚀 {len(standalone_plans)} Merkmale gehen auf die Standalone-Überholspur.")
    ...
    # =================================================================
    # PHASE 4: SYNTHESE & VALIDIERUNG (KI Code-Generierung)
    # =================================================================
    print("\n" + "="*50)
    ...
    for idx, plan in enumerate(all_final_plans):
        ...
        if len(target_name) > 30: # Fallback falls mehrere Merkmale gejoint wurden
    ...
    # Debug-Dump Phase 4
    try:
        with open("debug_3_final_datav15.json", "w", encoding="utf-8") as f:
...
if __name__ == "__main__":
    ...
    FULL_MODEL_JSON = "Beispiel1_Vorbedingungen_full.json" # Das große Wörterbuch
    TEST_SLICE_JSON = "Beispiel1_Vorbedingungen.json" # Der kleine 5-Merkmale Slice
    
    OUTPUT_EXCEL = "MAS_Entwickler_Dashboard_Finalv15.xlsx"
