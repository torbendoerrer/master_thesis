...
# ==========================================
# 2. WISSENSBASIS LADEN (RAG Layers)
# ==========================================
try:
...
try:
    with open('domain_knowledgev6.md', 'r', encoding='utf-8') as file:
...
def avc_format_validator(avc_code: str) -> str:
    ...
    # --- VALIDIERUNGSREGELN FÜR DEN SYNTHESIZER ---
    if "<echter_merkmals_name>" in code_lower:
        errors.append("[ERROR] Platzhalter <ECHTER_MERKMALS_NAME> im Code gefunden! Bitte durch das reale Ziel-Merkmal (z.B. K40_PS) ersetzen.")
    if " = ''" in code_lower or " = \"\"" in code_lower:
        errors.append("[ERROR] Leere Strings (= '') sind meistens Fehler. Nutze 'NOT pc.Merkmal SPECIFIED', wenn ein Feld leer sein soll.")
...
analyst = Agent(
    ...
    goal='Bereinige das LO-VC JSON. Ignoriere technische IDs, wende Kundendomänenwissen an und extrahiere NUR die echten Merkmale aus der Syntax.',
    backstory=f'''Du bist ein präziser Datenanalyst. Deine Aufgaben:
    1. Lösche Zeilen mit einem Stern (*), das sind alte Kommentare.
    2. WICHTIG: Das Feld "precondition" (z.B. V_TW0016) enthält nur technische Datenbank-IDs. Das sind KEINE Merkmale! Ignoriere diese IDs komplett.
    3. Lies ausschließlich das Feld "syntax". Extrahiere daraus die echten Merkmale, die den jeweiligen Wert steuern.
    4. GANZ WICHTIG: Wenn eine Bedingung/Syntax leer ist (oder fehlt), bedeutet das in SAP "NOT SPECIFIED". Interpretiere ein leeres Feld NIEMALS als den String-Wert '0'! Schreibe für solche Fälle explizit das Konzept "NOT SPECIFIED" in deine Liste.
...
architect = Agent(
    ...
    ZUDEM: Wenn der Analyst ein "NOT SPECIFIED" (oder ein leeres Feld) übergibt, übernehme genau dieses Konzept. Erfinde dafür niemals den Wert '0'.
    ABSOLUTE WERTE-TREUE: Verfälsche niemals die Originalwerte der Merkmale! Wenn ein Merkmal den Wert '0', '1', '3' oder 'd1' hat, schreibe EXAKT diese Werte in die Tabelle. Übersetze sie NIEMALS in 'Yes', 'No', 'Ja', 'Nein', 'True' oder 'False'!
    
    Berücksichtige für deine Herleitung (z.B. warum bestimmte Werte fehlen oder aussortiert wurden) zwingend folgendes Domänenwissen:
    --- START DOMÄNENWISSEN ---
    {domain_knowledge}
    --- ENDE DOMÄNENWISSEN ---
    ''',
...
synthesizer = Agent(
    ...
    {{
      ...
      "tabelle_zeilen": [
        ...
        ["NOT SPECIFIED", "Wert C", "Zielwert 2"]
      ],
      "avc_code": "TABLE VC_TAB_NAME (\\n  Spalte1 = pc.Spalte1\\n).\\npc.<ECHTER_MERKMALS_NAME> = 'Ausreißer' IF NOT pc.Spalte1 SPECIFIED."
    }}
    
    WICHTIGE ANWEISUNGEN ZUM JSON: 
    1. Ersetze "<ECHTER_MERKMALS_NAME>" in 'tabelle_kopf' und im 'avc_code' IMMER durch den echten Namen des Zielmerkmals (z.B. "K40_PS"). Nenne es NIEMALS "Ergebnis_Merkmal"!
    2. Achte darauf, dass "NOT SPECIFIED" aus der Architektur im Code korrekt als "NOT pc.Merkmal SPECIFIED" umgesetzt wird und NIEMALS als "pc.Merkmal = '0'".
    3. Achte darauf, dass das JSON gültig ist (korrekte Anführungszeichen entkommen).
    4. ABSOLUTES VERBOT FÜR LISTEN IN ZELLEN: Alle Elemente innerhalb von "tabelle_zeilen" MÜSSEN einfache Strings sein! Nutze NIEMALS verschachtelte Listen wie ["1", "2"]. Wenn mehrere Werte in eine Zelle gehören, schreibe sie als EINEN kommagetrennten String, z.B. "1, 2".
    5. WERTE-TREUE: Die Werte in der Tabelle MÜSSEN den originalen SAP-Werten entsprechen (z.B. '0', '1', '2', 'd1'). Übersetze diese NIEMALS in 'Yes', 'No', 'True', 'False', 'Ja' oder 'Nein'!''',
    verbose=True,
